 (function () {
    ...
    // ========= Telefon inputni faqat raqamga cheklash =========
    function sanitizePhone(raw){
      // faqat raqam va bitta boshidagi + ga ruxsat
      let v = raw.replace(/[^\d+]/g, "");
      // agar + bir nechta bo'lsa, birinchisini qoldirib qolganini olib tashlaymiz:
      if (v.indexOf("+") > 0) v = v.replace(/\+/g, "");
      if (v.indexOf("+") === 0) {
        // + dan keyingilarda faqat raqam
    ...
    // O'zbekiston formati: 998 + 9 ta raqam (umumiy 12 belgidan iborat)
    function isValidUzPhone(v){
      // + belgisi bo'lishi shart emas, lekin faqat raqam bo'lsin
      const only = v.startsWith("+") ? v.slice(1) : v;
      return /^998\d{9}$/.test(only);
    }

    // ========= Yuborish (Google Sheets) =========
    const SHEETS_URL = "https://script.google.com/macros/s/AKfycbxiS6639lAkwevuqShQ4I2_uuMhrz_ADO5lP7eq1iqpEajwfj3A0BQyAe9Ee6AJgphZag/exec";

    async function submitToSheets({name, phone}){
      const formData = new FormData();
      formData.append("Ism", name);
      formData.append("Telefon raqam", phone);
      // Asia/Tashkent bo'yicha vaqt
      const nowTashkent = new Date().toLocaleString("uz-UZ", { timeZone: "Asia/Tashkent" });
      formData.append("Royhatdan o'tgan vaqti", nowTashkent);

      const res = await fetch(SHEETS_URL, { method: "POST", body: formData, mode: "no-cors" });
      // Apps Script ko'pincha no-cors bilan ham yozadi; muvaffaqiyatni tekshirishning imkoni bo'lmaydi.
      // Shuning uchun yuborgan zahoti "rahmat" sahifasiga yo'naltiramiz.
      return true;
    }
    ...
    function bindForm(formId, nameId, telId, errNameId, errTelId, submitBtnId){
      ...
      nameEl.addEventListener("input", ()=> clearError(nameEl, errName));
      ...
      form.addEventListener("submit", async (e) => {
        ...
        try{
          await submitToSheets({name, phone});
          window.location.href = "thenkYou.html";
        } catch (err){
          submitBtn.disabled = false;
          submitBtn.textContent = formId === "modalForm" ? "→ BEPUL RO‘YXATDAN O‘TISH" : "-> BEPUL RO'YXATDAN O'TISH";
          alert("Uzr, yuborishda muammo yuz berdi. Birozdan so'ng qayta urinib ko'ring.");
        }
      });
    }
